...
{
	WMT_DETECT_PR_DBG("open major %d minor %d (pid %d)\n", imajor(inode), iminor(inode), current->pid);
...
{
	WMT_DETECT_PR_DBG("close major %d minor %d (pid %d)\n", imajor(inode), iminor(inode), current->pid);
...
static long wmt_detect_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	WMT_DETECT_PR_DBG("cmd (%d),arg(%ld)\n", cmd, arg);
	...
	case COMBO_IOCTL_DO_MODULE_INIT:
		...
		WMT_DETECT_PR_DBG("built-in mode\n");
		retval = do_connectivity_driver_init(arg);
#else
		WMT_DETECT_PR_DBG("kernel object mode\n");
...
int wmt_detect_ext_chip_pwr_on(void)
{
	...
	return 0;
#else
	WMT_DETECT_PR_DBG("combo chip is not supported\n");
	return -1;
#endif
}
...
int wmt_detect_ext_chip_detect(void)
{
	...
	if (bgfEintStatus == 0) {
		...
		WMT_DETECT_PR_DBG("external combo chip not detected\n");
		iRet = -2;
	} else if (bgfEintStatus == 1) {
		/*combo chip exists */
		WMT_DETECT_PR_DBG("external combo chip detected\n");

		/*detect chipid by sdio_detect module */
		chipId = sdio_detect_query_chipid(1);
		if (hif_sdio_is_chipid_valid(chipId) >= 0)
			WMT_DETECT_PR_DBG("valid external combo chip id (0x%x)\n", chipId);
		else
			WMT_DETECT_PR_DBG("invalid external combo chip id (0x%x)\n", chipId);
		iRet = 0;
	} else {
		/*Error exists */
...
static int wmt_detect_probe(struct platform_device *pdev)
{
	...
	WMT_DETECT_PR_DBG("platform name: %s\n", pdev->name);
...
static int wmt_detect_driver_init(void)
{
	...
	WMT_DETECT_PR_DBG("driver(major %d) installed success\n", gWmtDetectMajor);
