...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	if (Driver_Open_Count == 0) {
		...
		if (CodecHWLock.pvHandle != 0) {
			pr_debug("err vcodec_release %d, type = %d, 0x%lx\n",
				current->pid,
				CodecHWLock.eDriverType,
				(unsigned long)CodecHWLock.pvHandle);
...
{
	if (CodecHWLock.pvHandle != 0) {
		pr_debug("vcodec_suspend fail due to videocodec activity");
		return -EBUSY;
	}
					...
					unsigned long action, void *data)
{
	...
	pr_debug("vcodec_suspend_notifier ok action = %ld", action);
	switch (action) {
	case PM_SUSPEND_PREPARE:
		...
		while (CodecHWLock.pvHandle != 0) {
			wait_cnt++;
			if (wait_cnt > 90) {
				pr_debug("vcodec_pm_suspend waiting %p %d",
					CodecHWLock.pvHandle,
